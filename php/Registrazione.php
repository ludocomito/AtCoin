...
<head>
	...
	<!--Import del foglio di stile generale-->
	...
	<!--Definizione dello stile degli elementi specifici della pagina.-->
	<style>
		.titolo {
...
<body style="font-family: Helvetica, Sans-Serif">
	...
	<!--Definizione dell'header (desktop e responsive)-->
	<div id="menu">
	...
	<!--Definizione dell'alert in caso di registrazione con successo (normalmente nascosta, vedi riga 191)-->
	...
	<script>
		//  hide della schermata di registrazione effettuata
		$("#MyAlert").hide();
	</script>
	...
	<div id="MyPrincipale" class="center_div" style="margin-bottom: 10px;">
		...
		<h2 class="titolo">Registrazione Utente</h2>
		<p class="myLink"><a href="./Login.php">Sei già registrato? Clicca qui!</a> </p>

		<form action="<?php $_SERVER['PHP_SELF'] ?>" method="post">


			<!-- Definizione del form di registrazione -->
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($_POST['username']) && !empty($_POST['cognome']) && !empty($_POST['nome']) && !empty($_POST['nascita']) && !empty($_POST['email']) && !empty($_POST['psw']) && !empty($_POST['cpsw'])) {
				...
				//i campi non obbligatori vengono assrgnati o settati a NULL se non compilati
				...
				exit("<br><h3>Campi principali non settati !</h3>"); //errore per campi obbligatori non compilati

			//handling di errori su campi numerici
		...
		?>
			...
			//controlla unicità username ed handling erorri
			...
			} else { //inserimento dati utente su db
			...
			?>
				<script>
					//presentazione a schermo dell'alert di registrazione avvenuta
					$("#MyPrincipale").hide();
					$("#MyAlert").show("slow");
				</script>
			<?php
